...
(async () => {
  ...
  while (i < comics.length) {
    ...
    await sleep(6000);
    // 等待目标元素加载
    await page.waitForSelector(".halfGuide");
    console.log("333");
    ...
    await page.waitForSelector(".base-info > .title", { timeout: 600000 });
    // return;
    ...
    try {
      await page.goBack();
      // await page.waitForNavigation({ waitUntil: "domcontentloaded" }); // 等待导航完成
      // await sleep(6000);
      await page.reload({ waitUntil: "domcontentloaded" });

      // await page.waitForSelector(".base-info > .title", { timeout: 600000 });
    } catch (error) {
      console.error("Error during goBack, refreshing the page:", error);
      await page.reload();
    }
  }
})();
