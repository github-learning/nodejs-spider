...
(async () => {
  ...
  https: await page.goto("https://m.kuaikanmanhua.com/tag/0?region=1&sort=2", {
    timeout: 600000,
  });
    ...
    while (k < 3) {
      ...
      try {
        ...
        for (
          ...
          if (previousDir) {
            const filesInPreviousDir = fs.readdirSync(previousDir);
            const filesNeeded = 20 - filesInPreviousDir.length;

            // 如果上一个目录的文件数少于20，则先填充上一个目录
            if (filesNeeded > 0) {
              for (let i = 0; i < filesNeeded && group.length > 0; i++) {
                const imageUrl = group.shift(); // 从当前组中取出图片
                const imageFileName = `${imageCounter
                  .toString()
                  .padStart(4, "0")}.jpg`;
                const savePath = path.join(previousDir, imageFileName);

                try {
                  await downloadImage(imageUrl, savePath);
                  console.log(`Downloaded to previousDir: ${savePath}`);
                } catch (error) {
          ...
          if (group.length > 0) {
            ...
            ensureDirExists(thirdDir); // 确保新目录存在
